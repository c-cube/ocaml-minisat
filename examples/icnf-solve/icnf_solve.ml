  ...
  let mk_minisat : builder =
    {
      ...
      make =
        (fun () ->
          ...
          let solve ass =
            try
              S.solve ~assumptions:ass s;
              true
            with S.Unsat -> false
          in
          let unsat_core () = S.unsat_core s in
          let to_int i =
            S.Lit.to_int (S.Lit.abs i)
            *
            if S.Lit.sign i then
              1
            else
              -1
          in
          Solver { add_clause; unsat_core; solve; mklit; to_int });
    }
...
let solve_with_solver ~debug solver file : unit =
  ...
  let rec process_problem () =
    ...
    | Parse.Add_clause c ->
      if debug then Printf.printf "add_clause %a\n%!" pp_arr c;
    ...
    | Parse.Solve assumptions ->
      ...
      if not r then (
        let core = s.Solver.unsat_core () in
        if debug then Printf.printf "unsat core %a\n%!" pp_arr core;

        (* core must be subset of assumptions *)
        assert (
          Array.for_all
            (fun l -> Array.exists (fun l2 -> l = l2) assumptions)
            core);

        let r2 = s.Solver.solve core in
        if debug then
          Printf.printf "solving with only core: %s\n%!"
            (if r2 then
              "SAT"
            else
              "UNSAT");
...
let () =
  ...
  let opt_s =
    Arg.Symbol
      ( List.map Solver.name Solver.all,
        fun s ->
          solvers := Solver.(List.find (fun b -> b.name = s) all) :: !solvers )
  in
  let opts =
    [
  ...
  if !solvers = [] then solvers := [ Solver.mk_minisat ];
  (* default *)
