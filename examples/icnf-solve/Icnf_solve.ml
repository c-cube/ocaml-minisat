  ...
  let rec process_problem () =
    ...
    | Parse.Solve assumptions ->
      ...
      if not r then (
        ...
        if debug then (
          Printf.printf "unsat core %a\n%!" pp_arr core;
        );

        (* core must be subset of assumptions *)
        assert (Array.for_all (fun l -> Array.exists (fun l2 -> l=l2) assumptions) core);

        let r2 = s.Solver.solve core in
        if debug then (
          Printf.printf "solving with only core: %s\n%!" (if r2 then "SAT" else "UNSAT");
        );
        assert (not r2);
      );
...
let () =
  ...
  if !solvers=[] then solvers := [Solver.mk_minisat]; (* default *)
