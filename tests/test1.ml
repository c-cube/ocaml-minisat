...
let () =
  ...
  try
    Minisat.solve ~assumptions:[| l3 |] s;
    assert false
  with Minisat.Unsat ->
    print_endline "ok!";

    print_endline "should succeed...";
    let confl = Minisat.unsat_core s in
    Printf.printf "core has size %d\n" (Array.length confl);
    Array.iter
      (fun l -> Printf.printf "core contains %s\n" (Minisat.Lit.to_string l))
      confl;
    assert (confl = [| l3 |]);
    print_endline "ok...";
    ()
